...
class Command(BaseCommand):
    help = "Обход всех ингредиентов и получение для них цены: парсером или вручную"
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(project_dir):
            self.stderr.write(f"Директория с парсером не найдена: {project_dir}")
            return
        ...
        for name in ingredient_names:
            ...
            if ProductPrice.objects.filter(name__icontains=name).exists():
                self.stdout.write(self.style.SUCCESS("Цена уже есть в БД, пропускаем."))
                continue
            ...
            qs = ProductPrice.objects.filter(name__icontains=name).order_by('price')
            if qs.exists():
                price = qs.first().price
                self.stdout.write(self.style.SUCCESS(f"Парсер вернул цену: {price}"))
                continue

            self.stdout.write("Не удалось получить цену парсером.")
            while True:
                val = input(f"Введите цену для «{name}» (или напишите skip): ").strip()
                if val.lower() == 'skip':
                ...
                try:
                    ...
                    self.stdout.write(self.style.SUCCESS(f"Ручная установка: {price}"))
                    break
                except ValueError:
